...
// the character's head (main.cpp). So this costs one more draw call in the hud view and no
...
public:
    ...
    // time between two calls IS the frame, so this needs nothing from main.cpp's own clock --
...
private:
    ...
    // that writes a screenshot stalls to about 250 ms (main.cpp says so on the line that
